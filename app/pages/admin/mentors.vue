...
const filteredMentors = computed(() => {
  ...
  if (filterStatus.value !== 'all') {
    result = result.filter(m => (filterStatus.value === 'active' ? m.isActive : !m.isActive))
  }

  return result
})

const modalTitle = computed(() => (editingMentor.value ? 'Modifier le mentor' : 'Ajouter un mentor'))
...
async function handleSubmit() {
  ...
  if (editingMentor.value) {
    ...
    const index = mentors.value.findIndex(m => m.id === editingMentor.value!.id)
    if (index !== -1) {
      mentors.value[index] = {
        ...mentors.value[index],
  ...
  <div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
      <div>
        ...
        <p class="text-body text-gray-500">Gerez les mentors de la plateforme</p>
      </div>
    ...
    <!-- Table -->
    <UiCard>
      <UiTable :columns="columns" :data="filteredMentors" empty-text="Aucun mentor trouve">
        ...
        <template #name="{ item }">
          <div>
            <div class="text-body-sm font-medium text-gray-900">{{ item.name }}</div>
            <a
